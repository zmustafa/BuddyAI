...
public sealed class SuggestionEditorForm : Form
{
    ...
    private readonly List<SuggestionItem> _records = new();
    private bool _isLoading;
    private bool _isDirty;

    public SuggestionEditorForm(SuggestionService suggestionService)
    {
        _service = suggestionService;

        Text = "BuddyAI Suggestion Editor";
    ...
    private void BuildUi()
    {
        ...
        Label lblList = new() { Text = "Suggestions", Dock = DockStyle.Top, Height = 24 };
        left.Controls.Add(lblList);

        _txtSearch.Dock = DockStyle.Top;
        _txtSearch.PlaceholderText = "Search suggestions";
    ...
    private void BindList(string? selectId)
    {
        ...
        IEnumerable<SuggestionItem> filtered = _records;
        ...
        List<SuggestionItem> visibleRecords = filtered.ToList();

        _lstItems.BeginUpdate();
        _lstItems.Items.Clear();
        foreach (SuggestionItem record in visibleRecords)
    ...
    private void LoadSelectedRecordIntoEditors()
    {
        ...
        try
        {
            SuggestionItem? record = GetSelectedRecord();
            if (record == null)
            {
                ClearEditors();
                return;
    ...
    {
        SuggestionItem? selected = GetSelectedRecord();
        if (selected == null)
            return;

        SuggestionItem clone = CloneItem(selected);
    ...
    private void DeleteSelectedRecord()
    {
        ...
        if (MessageBox.Show(this, $"Delete '{selected.Name}'?", "Delete Suggestion", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes)
            return;
    ...
    private void MoveSelectedRecord(int direction)
    {
        ...
        SuggestionItem item = _records[current];
    ...
    private void ImportItems()
    {
        ...
        DialogResult merge = MessageBox.Show(this, "Merge imported items with the current working list? Click No to replace the current list.", "Import Suggestions", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
        if (merge == DialogResult.Cancel)
            return;

        List<SuggestionItem> imported = _service.Import(dialog.FileName, merge == DialogResult.Yes);
        _records.Clear();
    ...
    private void ExportItems()
    {
        using SaveFileDialog dialog = new()
        {
            ...
            FileName = "BuddyAI.suggestions.export.json",
            Title = "Export"
        };
    ...
    {
        foreach (SuggestionItem record in _records)
        {
    ...
    private SuggestionItem? GetSelectedRecord()
    {
    ...
    {
        Text = _isDirty ? "BuddyAI Suggestion Editor *" : "BuddyAI Suggestion Editor";
    }

    private static SuggestionItem CloneItem(SuggestionItem item)
    {
    ...
    {
        public SuggestionItem Record { get; }

        public ItemListEntry(SuggestionItem record)
        {
            Record = record;
        }
