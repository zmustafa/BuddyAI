...
public sealed class AiProviderManagerForm : Form
{
    ...
    public AiProviderManagerForm(AiProviderService providerService)
    {
        _providerService = providerService;

        Text = "BuddyAI AI Provider Manager";
    ...
    private void BuildUi()
    {
        ...
        leftToolbar.Items.AddRange(new ToolStripItem[] { _btnNew, _btnClone, _btnDelete, new ToolStripSeparator(), _btnReload, _btnOpenJson, _btnExportImport });
    ...
    private void InitializeTreeImageList()
    {
        ...
        // Provider icon: a simple server/plug shape
        ...
        // Model icon: a small diamond/chip shape
    ...
    private void WireEvents()
    {
        ...
        _btnExportWithoutKeys.Click += (_, _) => ExecuteGuarded(() => ExportProviders(includeApiKeys: false), "Export providers");
        ...
        FormClosing += AiProviderManagerForm_FormClosing;
    }
    ...
    private void AiProviderManagerForm_FormClosing(object? sender, FormClosingEventArgs e)
    {
        if (!_isDirty)
            return;
        ...
        DialogResult result = MessageBox.Show(
            this,
            "You have unsaved provider changes. Save before closing?",
            "AI Provider Manager",
    ...
    private void LoadRecords(string? selectId)
    {
        ...
        try
        {
            ...
            _lblPath.Text = "JSON file: " + _providerService.GetStoragePath();
        }
        finally
        {
            _isLoading = false;
    ...
    private void RebuildTree(string? selectId)
    {
        ...
        foreach (AiProviderDefinition record in _records)
        {
            ...
            TreeNode providerNode = new(providerLabel) { Tag = record, ImageIndex = ProviderImageIndex, SelectedImageIndex = ProviderImageIndex };

            foreach (AiProviderModelDefinition model in AiProviderService.NormalizeModels(record.Models, record.ProviderType))
            {
                string modelLabel = model.Name;
                TreeNode modelNode = new(modelLabel) { Tag = model, ImageIndex = ModelImageIndex, SelectedImageIndex = ModelImageIndex };
    ...
    private void SetModelRows(IEnumerable<AiProviderModelDefinition> models)
    {
        string providerType = string.IsNullOrWhiteSpace(_cmbProviderType.Text)
            ? (_records.FirstOrDefault(x => string.Equals(x.Id, _loadedRecordId, StringComparison.OrdinalIgnoreCase))?.ProviderType ?? AiProviderTypes.OpenAI)
            : _cmbProviderType.Text.Trim();

        _modelRows = new BindingList<ModelGridRow>(AiProviderService.NormalizeModels(models, providerType)
            .Select(x => ModelGridRow.FromModel(x))
            .ToList());
        _gridModels.DataSource = _modelRows;
    }
    ...
    private void CommitEditorsToLoadedRecord()
    {
        ...
        AiProviderDefinition? record = _records.FirstOrDefault(x => string.Equals(x.Id, _loadedRecordId, StringComparison.OrdinalIgnoreCase));
        if (record == null)
            return;
    ...
    private void RefreshSelectedProviderNode(AiProviderDefinition record)
    {
        ...
        foreach (AiProviderModelDefinition model in AiProviderService.NormalizeModels(record.Models, record.ProviderType))
        {
            providerNode.Nodes.Add(new TreeNode(model.Name) { Tag = model, ImageIndex = ModelImageIndex, SelectedImageIndex = ModelImageIndex });
        }
    ...
    private void ImportViaWizard()
    {
        ...
        using Forms.ProviderImportWizardForm wizard = new();
        if (wizard.ShowDialog(this) == DialogResult.OK && wizard.ImportedProvider != null)
        {
            AiProviderDefinition record = wizard.ImportedProvider;
            _records.Add(record);
            _isLoading = true;
            try
            {
    ...
    private void ImportOpenAi()
    {
        ...
        using Forms.OpenAiImportForm importForm = new();
    ...
    private void AddDefaultProviders()
    {
        ...
        {
            // Give them new IDs in case there's already one in the list
    ...
    private void ImportGrok()
    {
        ...
        using Forms.GrokImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            if (string.IsNullOrWhiteSpace(importForm.ImportedApiKey))
                return;
    ...
    private void ImportClaude()
    {
        ...
        using Forms.ClaudeImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            if (string.IsNullOrWhiteSpace(importForm.ImportedApiKey))
                return;
    ...
    private void ImportGemini()
    {
        ...
        using Forms.GeminiImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            if (string.IsNullOrWhiteSpace(importForm.ImportedApiKey))
                return;
    ...
    private void ImportMistral()
    {
        ...
        using Forms.MistralImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            if (string.IsNullOrWhiteSpace(importForm.ImportedApiKey))
                return;
    ...
    private void ImportOpenAIOAuth()
    {
        ...
        using Forms.ChatGPTOAuthImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            AiProviderDefinition record = new()
            {
    ...
    private void ImportClaudeOAuth()
    {
        ...
        using Forms.ClaudeOAuthImportForm importForm = new();
        if (importForm.ShowDialog(this) == DialogResult.OK)
        {
            AiProviderDefinition record = new()
            {
    ...
    private void ReauthenticateOAuth()
    {
        ...
        if (providerType == AiProviderTypes.ClaudeOAuth)
        {
            ...
            if (importForm.ShowDialog(this) == DialogResult.OK)
            {
                MessageBox.Show(this, "Re-authentication completed successfully.", "AI Provider Manager", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        else
        {
    ...
    private void ExportProviders(bool includeApiKeys)
    {
        ...
        if (_records.Count == 0)
        {
            MessageBox.Show(
                this,
                "There are no providers to export.",
                "AI Provider Manager",
        ...
        {
            Title = includeApiKeys ? "Export Providers (with API keys)" : "Export Providers (without API keys)",
    ...
    {
        if (_gridModels.CurrentRow == null || _gridModels.CurrentRow.Index < 0 || _gridModels.CurrentRow.Index >= _modelRows.Count)
            return;
    ...
    {
        Text = _isDirty ? "BuddyAI AI Provider Manager *" : "BuddyAI AI Provider Manager";
    }
